...
int main()
{
	...
	char s2_q2[100];
	puts("What do you do at your occupation?\nCondense the verb in your answer into a single -er noun. (e.g. if you are a sheep farmer, your answer will be “farmer”)");
	scanf("\n%[^\n]s", &s2_q2);

	char s2_q3[100];
	puts("What was your first pet’s specific species/breed? If you never had a pet, please answer with an animal you wish you owned.");
	scanf("\n%[^\n]s", &s2_q3);

	char s2_q4[100];
	puts("What’s your most embarrassing childhood memory?\nCondense this story into two words");
	scanf("\n%[^\n]s", &s2_q4);

	char s2_q5[100];
	puts("What is the object you’d least like to be stabbed by?");
	scanf("\n%[^\n]s", &s2_q5);

	char s2_q6[100];
	puts("What is something you are good at? (Verb ending in -ing)");
	scanf("\n%[^\n]s", &s2_q6);
	char s2_q7[100];
	puts("How many carrots do you believe you could eat in one sitting, if someone, like, forced you to eat as many carrots as possible?");
	scanf("\n%[^\n]s", &s2_q7);
	char s2_q8[100];
	puts("What is your greatest intangible fear? (e.g. death, loneliness, fear itself)");
	scanf("\n%[^\n]s", &s2_q8);
	char s2_q9[100];
	puts("What is your greatest tangible fear? (e.g. horses)");
	scanf("\n%[^\n]s", &s2_q9);
	char s2_q10[100];
	puts("What is the last thing you did before starting this worksheet?");
	scanf("\n%[^\n]s", &s2_q10);


	char s2_q11[100];
	puts("What condition is your body currently in? (single word answer)");
	scanf("\n%[^\n]s", &s2_q11);
	

	char s2_q12[100];
	puts("Favorite state of matter?");
	scanf("\n%[^\n]s", &s2_q12);
	char s2_q13[100];
	puts("A word your name kind of sounds like? (e.g. Brian -> Brain)");
	scanf("\n%[^\n]s", &s2_q13);
	char s2_q14[100];
	puts("What is your Zodiac sign?");
	scanf("\n%[^\n]s", &s2_q14);
	char s2_q15[100];
	puts("If you had to define your personality in one word, what would it be?");
	scanf("\n%[^\n]s", &s2_q15);
	//section 3
	char s3_q16[100];
	puts("Who is your favorite film character? (NOTE: must be played by Kurt Russell)");
	scanf("\n%[^\n]s", &s3_q16);
	char s3_q17[100];
	puts("What is the last word of the title of your favorite Kubrick film?");
	scanf("\n%[^\n]s", &s3_q17);
	char s3_q18[100];
	puts("What is the first word in the title of your favorite Joy Division album?");
	scanf("\n%[^\n]s", &s3_q18);
	char s3_q19[100];
	puts("What is a scientific term you picked up from listening to NPR once?");
	scanf("\n%[^\n]s", &s3_q19);
	char s3_q20[100];
	puts("What is a piece of military hardware you think looks cool even though war is bad?");
	scanf("\n%[^\n]s", &s3_q20);
	char s3_q21[100];
	puts("What is something you’d enjoy watching Mads Mikkelsen do?\nPlease condense into one word.");
	scanf("\n%[^\n]s", &s3_q21);
